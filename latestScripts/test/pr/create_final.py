...
os.chdir(r"D:/Coding/2017/ccbd/MIXED3_mixed up stuff/mixCounters5/mixCounters5/")
mapTime = []
mapResources = []
mapID = []
jobName = []

ff = open("../pr/paramfile.txt","w")

for folder in os.listdir("D:/Coding/2017/ccbd/MIXED3_mixed up stuff/mixCounters5/mixCounters5/"):

    params = folder.split("-")
    #ff.write(params[1] + "," + params[2] + "," + params[3] + "," + params[4] + "," + params[5] + "," + params[6] + "," + "\n")
    z = "D:/Coding/2017/ccbd/MIXED3_mixed up stuff/mixCounters5/mixCounters5/" + folder + "/pagerank"
                    ...
                    if i['counterGroupName'] == "org.apache.hadoop.mapreduce.TaskCounter":
                        ...
                        for j in mapCounter:
                            ...
                            if j['name'] == 'COMMITTED_HEAP_BYTES':
                                tempMapRes.append(j['value'])
                            if j['name'] == 'MAP_INPUT_RECORDS':
                                tempMapRes.append(j['value'])
                            if j['name'] == 'MAP_OUTPUT_RECORDS':
                                tempMapRes.append(j['value'])
...
os.chdir(r"D:/Coding/2017/ccbd/MIXED3_mixed up stuff/mixCounters5/pr")
'''
...
f2 = open("clusternumbers.txt", "w")
for i in range(len(mapResources)):
    f1.write(','.join(list(map(str,mapResources[i]))) + "\n") 
    f2.write(str(timeClusterList[i]) + "\n") # str(mapTime[i]) +','+ str(timeClusterList[i]) + "\n")
