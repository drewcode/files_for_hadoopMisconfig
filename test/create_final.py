...
os.chdir(r"D:/Coding/2017/ccbd/NEW PAYLOAD/run2/xml")
mapTime = []
mapResources = []
mapID = []
jobName = []
...
for folder in os.listdir():
    ...
    for subfolder in ["job1","job2","job3","job4","job5","job6"]:
        z = "D:/Coding/2017/ccbd/NEW PAYLOAD/run2/xml/" + folder + "/" + subfolder
        if z == 'D:/Coding/2017/ccbd/NEW PAYLOAD/run2/xml/combo-4096-4096-4-4-6.5-30-/job6':
            continue
        os.chdir(z)
...
os.chdir(r"D:/Coding/2017/ccbd/NEW PAYLOAD/run2")
...
ff = open("clusternumbers.txt", "w")
for i in range(1600):
    f.write(','.join(list(map(str,mapResources[i]))) + "\n") # str(mapTime[i]) +','+ str(timeClusterList[i]) + "\n")
    ff.write(str(timeClusterList[i]) + "\n")
